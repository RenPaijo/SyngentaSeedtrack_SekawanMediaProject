...
export class Method {
  searchAction() {
    cy.get(locator.inputSearch).click().type('CFC');
    cy.get(locator.tablePp).should('contain', 'CFC');
    cy.get(locator.tablePp).find(locator.rowTable).should('have.length', 3);
    cy.get(locator.rowTable).eq(3).should('contain', 'CFC');
  }

  searchUnexistData() {
    cy.get(locator.inputSearch).click().type('no data');
    cy.get(locator.tablePp).should('contain', 'No Data Available');
  }
